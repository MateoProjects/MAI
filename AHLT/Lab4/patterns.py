   ...
   if tkE1 is not None and tkE2 is not None:
      ...
      if tree.get_tag(lcs)[0:2] == "VB" and tree.get_word(lcs-1) != "not":
         ...
         if (func1=='nsubj' and func2=='obj') or (func1=='obj' and func2=='nsubj'):
            ...
            'potentiate','enhance',]: # reduce, antagonize are ambiguous
               return 'effect'
            if lemma in ['impair','inhibit','displace','accelerate','bind','induce','elevate','delay']:   # DECREASE, inhibit, displace, induce, elevate, delay are ambiguous
               return 'mechanism'
            if lemma in ['exceed']:
               return 'advise'
      ...
      for t in range(tkE1+1,tkE2):
         ...
         if r1 < l and r < l2 and tree.get_word(t-1) != "not":
            ...
            'augment','accentuate','exacerbate','proliferation','rarely','secondary']: # proliferation, rarely, secondary added
               return 'effect'
            if lemma in ['react','faster','presumably','induction','modest','induction']: # delay, induce, modest, induction added | DELAY, INDUCE, substantially, minimally are ambiguous
               return 'mechanism'
            if lemma in ['exceed','extreme','cautiously']: # should added | SHOULD is ambiguous
               return 'advise'
   ...
   if tkE1 is not None and tkE2 is not None:
      ...
      if tree.get_tag(lcs)[0:2] in ["VB","NN"] and tree.get_word(lcs-1) != "not":
         ...
         if lemma in ['accentuate', 'counteract', 'augment', 'combine', 'diminish',
          'regulate', 'enhance']: # injection, potentiate are ambiguous
            return 'effect'
         if lemma in ['impair']:  # accelerate and react may be ambiguous
            return 'mechanism'
...
#  -- Designed to solve ambiguities   # TODO
   ...
   if tkE1 is not None and tkE2 is not None:
      ...
      if tree.get_tag(lcs)[0:2] == "VB" and tree.get_word(lcs-1) != "not":
         ...
         if func1 is not None and func2 is not None:
            ...
            # Some particular cases # TODO
            if lemma_f1_f2 in ['reduce_obl_nsubj','accentuate_nsubj:pass_obl', 'potentiate_nsubj:pass_obl']:
               return "effect"
            if  lemma_f1 in ['antagonize_nsubj:pass', 'antagonize_csubj:pass']:
               return "effect"
            if lemma_f1_f2 in ['increase_nsubj:pass_obl', 'impair_nsubj:pass_obl']:
               return "mechanism"
